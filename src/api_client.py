...
# credentials — это ваши AuthData из личного кабинета Сбера
giga_client = GigaChat(
    credentials= 'MDE5ZDk3NmYtZjU3OS03NjFhLWJiMTYtNGVmMjdiYzc0N2FhOjQzZDQzYjQ2LWQ5NGItNGM0My1iZGU5LTExMTczMDhmNTAwNQ==',#os.getenv("GIGACHAT_CREDENTIALS"), 
    verify_ssl_certs=False
)
...
async def get_recommendations(dish_title):
    """Запрашивает рекомендации по напиткам и приправам у GigaChat"""
    try:
        prompt = (
            f"Ты эксперт-сомелье и шеф-повар. К блюду '{dish_title}' подбери: "
            f"1. Идеально подходящий напиток (алкогольный или безалкогольный). "
            f"2. Одну-две приправы, которые раскроют вкус. "
            f"Ответь коротко и аппетитно в 2-3 предложениях."
        )
        response = giga_client.chat(prompt)
        return response.choices[0].message.content
    except Exception as e:
        logging.error(f"Ошибка GigaChat (рекомендации): {e}")
        return "Не удалось подобрать рекомендации, но это блюдо точно будет вкусным! 😉"